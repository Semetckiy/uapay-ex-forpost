...
export class KeyComponent implements OnInit {
  ...
  loadTotal(name: string, lang: string, namelang: string) {
    this.http.get(`/api/dictionary/?lang=ENG&name_like=${this.name}&namelang_like=${this.namelang}`).subscribe((x:any) => {
  ...
  loadPage(page: number, itemsPerPage: number, name: string, lang: string, namelang: string, sort: string, order: string) {
    ...
    //
    this.http.get(`/api/dictionary/?_page=${page}&_limit=${this.itemsPerPage}&lang=ENG&name_like=${this.name}&namelang_like=${this.namelang}&_sort=${this.sort}&_order=${this.order}`).subscribe((x:any) => {
      this.keys = x;
      console.log(`/api/dictionary/?_page=${page}&_limit=${this.itemsPerPage}&lang=ENG&name_like=${this.name}&namelang_like=${this.namelang}&_sort=${this.sort}&_order=${this.order}`);
    });
  }
