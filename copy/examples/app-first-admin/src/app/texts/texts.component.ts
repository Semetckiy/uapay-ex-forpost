...
export class TextsComponent implements OnInit {
  ...
  loadTotal(countryCode: string, lang: string, mode: string, text: string) {
    this.http.get(`/api/siteTexts/?countryCode=UA&lang_like=${this.lang}&mode_like=${this.mode}&text_like=${this.text}`).subscribe((x:any) => {
  ...
  loadPage(page: number, itemsPerPage: number, countryCode: string, lang: string, mode: string, text: string, sort: string, order: string) {
    ...
    //
    this.http.get(`/api/siteTexts/?_page=${page}&_limit=${this.itemsPerPage}&countryCode=UA&lang_like=${this.lang}&mode_like=${this.mode}&text_like=${this.text}&_sort=${this.sort}&_order=${this.order}`).subscribe((x:any) => {
      this.texts = x;
      console.log(`/api/siteTexts/?_page=${page}&_limit=${this.itemsPerPage}&countryCode=UA&lang_like=${this.lang}&mode_like=${this.mode}&text_like=${this.text}&_sort=${this.sort}&_order=${this.order}`);
    });
  }
